...
async def test_is_bounty_label_threshold(tmp_path):
    """Mock the DB rows and call dump_dataset with label_threshold=50.0; assert a $40 row gets is_bounty='' (ambiguous) and a $60+vibe row gets is_bounty=1."""
...
async def test_high_amount_is_positive_even_if_unscored(tmp_path):
    ...
    Assert is_bounty == '' (ambiguous because unscored).
    """
